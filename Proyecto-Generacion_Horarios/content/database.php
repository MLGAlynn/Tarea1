...
class Database{
    ...
    private $dbuser = "root";
    private $dbpass = "";
    private $dbname = "horario_escuela";
    ...
    public function altadocentes($tGradoAcademico,$tNombreDocente,$tApellidoPaterno,$tApellidoMaterno,$tCorreoElectronico,$eTelefono){
    ...
    public function listadodocente(){
        $sql = "SELECT * FROM cat_docente";
        $res = mysqli_query($this->con,$sql);
        return $res;
    }
    public function borrardocente($eCodDocente){
        $sql ="DELETE from cat_docente where eCodDocente = $eCodDocente";
        $res =mysqli_query($this->con,$sql);
        if ($res){
            return true;
        }
